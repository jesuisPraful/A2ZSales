...
namespace A2ZSalesDataAccessLayer
{
    public class A2ZSalesRepository
    {
        public A2zsalesAngularDbContext Context { get; set; }

        public A2ZSalesRepository()
        {
            Context = new A2zsalesAngularDbContext();
        }
        ...
        public List<PurchaseDetail> DisplayPurchaseDetailsByCustomer(string emailId)
        {
            ...
            try
            {
                lstPurchaseDetails = Context.PurchaseDetails
                    .Include(x => x.Product)
                    .Where(x => x.EmailId == emailId)
                    .OrderByDescending(x => x.DateOfPurchase)
                    .Select(x => x).ToList<PurchaseDetail>();
            }
            catch (Exception ex)
            {
        ...
        {
            List<CartProduct> lstProduct = null;
            try
            {
                SqlParameter prmEmailId = new SqlParameter("@EmailId", emailId);
                lstProduct = Context.CartProducts
                                    .FromSqlRaw("SELECT * FROM dbo.ufn_FetchCartProductByEmailId(@EmailId)", prmEmailId)
                                    .ToList();
            }
            catch (Exception ex)
            {
