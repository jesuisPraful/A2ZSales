...
WHERE ('[' + name + ']' = N'A2ZSalesAngularDB' OR name = N'A2ZSalesAngularDB')))
DROP DATABASE A2ZSalesAngularDB

CREATE DATABASE A2ZSalesAngularDB
GO
Use A2ZSalesAngularDB
GO
...
(
	...
	[ProductName] VARCHAR(50) CONSTRAINT fk_Rating_Products_ProductName FOREIGN KEY references Products(ProductName),
...
BEGIN TRY 
	...
	IF NOT(@ReviewRating>=0 and @ReviewRating<=5)
		SET @RetVal = -3
	INSERT INTO Rating VALUES(@EmailId,@ProductId,@ProductName,@ReviewRating,@ReviewComments)
	SET @RetVal = 1
	RETURN @RetVal
END TRY
BEGIN CATCH
...
INSERT INTO CardDetails(CardNumber,NameOnCard,CardType,CVVNumber,ExpiryDate,Balance) VALUES(2099299687852320,'Carlos González','V',244,DATEADD(YEAR, 10,'2026-01-07'),7330.00)
